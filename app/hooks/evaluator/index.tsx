...
import {
  EvaluatorDataModel,
  EvaluatorPayloadCreateModel,
  EvaluatorPayloadUpdateModel,
} from "@/app/models/evaluator";

const baseUrl = "/api/admin/dashboard/evaluator";
const entity = "evaluator";
const queryKey = "evaluators";

export const useEvalutors = ({ queryString }: { queryString?: string }) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as EvaluatorDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: EvaluatorPayloadCreateModel) =>
...
export const useEvalutor = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
  ...
  const { mutateAsync: onUpdate, isPending: onUpdateLoading } = useMutation({
    mutationFn: async ({
      ...
      id: string;
      payload: EvaluatorPayloadUpdateModel;
