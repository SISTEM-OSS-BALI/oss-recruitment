...
import { CardTemplateDataModel, CardTemplatePayloadCreateModel, CardTemplatePayloadUpdateModel } from "@/app/models/card-template";

const baseUrl = "/api/admin/dashboard/card-template";
const entity = "card-template";
const queryKey = "card-templates";

export const useCardTemplates = ({ queryString }: { queryString?: string }) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as CardTemplateDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: CardTemplatePayloadCreateModel) =>
...
export const useCardTemplate= ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
