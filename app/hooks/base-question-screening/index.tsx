...
const baseUrl = "/api/admin/dashboard/base-question-screening";
const entity = "base-question-screening";
const queryKey = "base-question-screening";

export const useQuestionBaseScreenings = ({
  queryString,
}: {
  queryString?: string;
}) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as QuestionBaseScreeningDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: QuestionBaseScreeningPayloadCreateModel) =>
...
export const useQuestionBaseScreening = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
