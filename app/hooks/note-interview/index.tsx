...
import {
  NoteInterviewDataModel,
  NoteInterviewPayloadCreateModel,
  NoteInterviewPayloadUpdateModel,
} from "@/app/models/note-interview";

const baseUrl = "/api/admin/dashboard/note-interview";
const entity = "note-interview";
const queryKey = "note-interviews";

export const useNoteInterviews = ({
  queryString,
}: {
  queryString?: string;
}) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as NoteInterviewDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: NoteInterviewPayloadCreateModel) =>
...
export const useNoteInterview = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
